...
La première partie décrit les conditions initiales du territoire simulé. La deuxième partie décrit les résultats de simulation.
...
Le nombre de polygones simulés est de `r nrow(forestStands)`, pour une surface totale de `r round(sum(forestStands$AREA)/10000,-1)` ha. Leurs caractéristiques sont présentées dans le paragraphe suivant.

# Description générale du territoire

## Typologie des polygones

La typologie des polygones résulte d'un croisement entre la propriété (qui oriente partiellement la sylviculture) et la composition en essence (qui détermine le paramétrage de la croissance et oriente partiellement la sylviculture).

La répartition en pourcentage de la surface des types de polygones est présentée sur le graphique suivant. La carte montre leur répartition sur le territoire.
...
Selon les conditions d'accessibilité du polygone, la propriété et la surface moyenne des parcelles cadastrales qu'il contient (pour la propriété privée), il peut être géré ou non. La répartition des surfaces selon l'accessibilité et la gestion est présentée sur le graphique suivante.
...
La répartition des types de polygones selon la gestion (ou non) est présentée dans les figures ci-dessous.
...
Différent itinéraires sylicoles sont possibles (cf document sur la typologie / les itinéaires, à rédiger avec FCBA). À chaque polygone est affecté un itinéraire sylvicole, en respectant une certaine répartition des polygones dans les itinéraires selon leur type. Le paramétrage des itinéraires, notamment le diamètre objectif, dépend de la typologie des polygones et de leur composition en essence.
...
La répartition des itinéraires par type de polygone figure ci-dessous. Cette répartition pour le scénario de référence est basée sur une analyse réalisée par FCBA sur les placettes PROTEST et IFN.
...
## Dendrométrie par itinéraire et type de polygone

Les graphiques suivants montrent sous forme de boîtes à moustaches les valeurs initiales de RDI (indice de densité relatif correspondant à la densité du peuplement divisée par la densité maximale permise par la loi d’auto-éclaircie) et de Dg (diamètre quadratique moyen d’un peuplement) pour les itinéraires et sous-itinéraires appliqués aux différents types de polygones. La largeur des boîtes est proportionnelle à la surface totale des polygones de chaque type.  

La ligne rouge pointillée sur le graphique de RDI correspond au maximum possible (1). Au dessus de cette valeur, la densité est réduite par mortalité. Les lignes bleues en pointillés sur les graphiques de RDI correspondent à la plage de densité objectif. Lorsque que la valeur supérieure de RDI autorisée est atteinte, une éclaircie est déclenchée afin de ramener le RDI à la valeur inférieure. Plusieurs lignes peuvent figurer dans les cas où des sous-itinéraires (définis par des plages de densités différentes) existent.

* Amélioration puis coupe rase : les sous-itinéraires sont caractérisés par des plages de RDI autorisées de 0,6 $\pm$ 0,05 et 0,7 $\pm$ 0,05. L'éclaircie se fait plutôt parmi les arbres les plus petits.
- Coupe rase : la coupe est déclenchée uniquement quand le diamètre objectif est atteint. Le RDI est maintenu au-dessous de 1 par mortalité.
- Irrégulier : les trois sous-itinéraires sont caractérisés par des plages de RDI autorisées de 0,5 $\pm$ 0,1 (clair), 0,7 $\pm$ 0,1 (moyen) et 0,9 $\pm$ 0,1 (dense). L'éclaircie se fait plutôt parmi les arbres les plus gros.
...
- Th1 et Thé : amélioration clair et dense.
...
Les histogrammes suivantes montrent la distribution de l'intensité des interventions, en volume, et en surface par itinéraire.
...
g1 <- ggplot(data=df[which(df$volumeRemoved>0),], aes(x=volumeRemoved_m3, weights=volumeRemoved_m3*AREA/10000, color=domainType)) +
...
ggplot(data=forestStands[which(forestStands$itineraire!="Inaction"),], aes(x=mean.volume.per.year, weights=AREA/10000)) +
  geom_histogram(breaks=seq(from=0, by=2.5, to=ceiling(max(forestStands$mean.volume.per.year)/2.5)*2.5)) +
  labs(fill="") + labs(y="Surface (ha)", x="Exploitation moyenne sur la période (m3/ha/an)")
