...
Afin de tenir compte des essences présentes sur la zone et de celles qui peuvent être modélisées par le simulateur, il a été décidé de simuler les peuplements suivants :
...
* diamètre quadratique moyen ($D_g$, cm) des arbres plus de 7,5 cm de diamètre ;
...
La densité de tiges est calculées comme : $N = {{4 G} \over {\pi {D_G}^2}}$
...
Le diamètre quadratique moyen pour chaque polygone est obtenu par la formule suivante : $D_G = \sqrt{\sum{N_i \times {D_{Gi}}^2} \over \sum{N_i}}$ avec ${i}$ les pixels contenus dans le polygone.
...
Les polygones dont le $D_G$ est nul sont également enlevés. Il s'agit de quelques polygones principalement situés dans les vallées périphériques du PNR.
...
Le champ CODEGRECO de cette couche vecteur est rasterisé au pas de 5 m. La valeur CODEGRECO affectée à chaque polygone correspond au mode des pixels qui sont contenus par le polygone.
...
- de même, quand une espèce feuillue autre que chêne et hêtre est présente, sa surface terrière est reporté au chêne et au hêtre selon les proportions relatives de ces deux espèces dans la placette. Ceci à l'exception des espèces de chêne qui sont toujours reportées sur  \textit{Quercus petraea}.
Les perches ont été prises en compte dans ce calcul. Les perches viables de feuillus et les brins de taillis ont été considérés comme des feuillus indeterminés. Les perches résineuses viables ont été considérées comme des conifères indeterminés. Les perches non viables, sans information d'espèce, ont été reportées sur les perches feuillues et résineuses selon les proportions relatives de ces dernières. Les polygones dont la composition n'entre pas dans les sept compositions proposées ont été enlevés.
...
avec $G_{sp}$, $D_{sp}$ et $n_{sp}$ la surface terrière, le diamètre quadratique et le nombre de tige par espèce, et  
$Dg_t^2 = \frac{n_{sp1}Dg_{sp1}^2 + n_{sp2}Dg_{sp2}^2}{n_{sp1} + n_{sp2}}$  
avec $Dg_t$ diamètre quadratique total; $Dg_{sp1}$ et $Dg_{sp2}$, les diamètres quadratiques des espèces $sp1$ et $sp2$; et $n_{sp1}$ et $n_{sp2}$, le nombre de tiges des espèces $sp1$ et $sp2$. $Dg_{sp1}$ et $Dg_{sp1}$ sont donc exprimés comme :
