...
Le volume récolté correspond dans le simulateur au volume des arbres coupés. Il s'agit du volume sur écorce du tronc jusqu'à la découpe 7 cm. On entend par surface traitée la superficie des polygones où a eu lieu une intervention sylvicole de coupe (éclaircie, coupe finale ou rase, coupe en irrégulier).
...
Les tableaux ci-dessous montrent la surface gérée, la surface traitée en moyenne annuelle, la récolte totale en moyenne annuelle, et la récolte moyenne en m^3^ par an et par hectare de forêt gérée. Le premier présente les chiffres en valeur absolue, le second en relatif (pourcentage) par rapport au scénario "PTR".
...
names(recap) <- c("Scénario", "Surface gérée (ha)", "Surface traitée (ha/an)", "Récolte moyenne (m3/ha/an)", "Récolte totale (m3/an)")
recap.relatif <- recap
recap[, "Surface gérée (ha)"] <- round(recap[, "Surface gérée (ha)"], -2)
recap[, "Surface traitée (ha/an)"] <- round(recap[, "Surface traitée (ha/an)"], -1)
recap[, "Récolte moyenne (m3/ha/an)"] <- round(recap[, "Récolte moyenne (m3/ha/an)"], 1)
recap[, "Récolte totale (m3/an)"] <- round(recap[, "Récolte totale (m3/an)"], -3)
...
La répartition de la récolte sur le territoire est présentée sur la carte suivante. Les zones en gris sont les polygones en inaction.
...
g <- ggplot() + geom_sf(data=dummy, aes(fill=totalVolumeRemoved_m3.ha), color = NA) + theme(legend.position="bottom") +
  ...
  labs(fill = paste0("Récolte sur ", duration, " ans (m3/ha)"))
print(g)
```
...
En public, la récolte se fait majoritairement en sapin-épicéa, et en mixte. Il y a moins de peuplements feuillus qu'en privé.
